...
func init() {
	...
	shell.Add(shell.Cmd{
		...
		Syntax:  "(verbose)?",
		Help:    "list UEFI variables (enhances user prompt)",
		Fn:      efivarCmd,
	})
}
...
func uefiCmd(_ *shell.Interface, _ []string) (res string, err error) {
	...
	p += "Start by reading the function pointer tables at the Runtime Services and Boot Services addresses above to confirm signatures and map all handler addresses.\n"
	p += "Then proceed with the methodology."
	user = fmt.Sprintf("%s\n%s", p, user)
...
func efivarCmd(_ *shell.Interface, arg []string) (res string, err error) {
	...
	// enhance user prompt
	user = fmt.Sprintf("These are the current session EFI variables:\n%s\n%s", buf.String(), user)
